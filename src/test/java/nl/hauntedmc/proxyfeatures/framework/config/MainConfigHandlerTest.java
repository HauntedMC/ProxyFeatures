...
class MainConfigHandlerTest {
    ...
    @Test
    void cleanupUnusedFeaturesWithoutSectionLeavesExistingConfigUntouched() {
        MainConfigHandler handler = createHandler();
        ...
        assertNull(handler.get("features.keep.enabled"));
    }
    ...
    void reloadConfigPreservesPersistedValues() {
        MainConfigHandler handler = createHandler();
        handler.put("global.server_name", "haunted-proxy");
        assertEquals("haunted-proxy", handler.getGlobalSetting("server_name", String.class));
    ...
    @Test
    void primaryConstructorLoadsDefaults() {
