...
class VotifierServerTest {
    ...
    void readExactWithDeadlineReturnsExactBytesAndFailsOnShortOrOversized() throws Exception {
        ...
        byte[] exact = readExact(server, new ByteArrayInputStream(new byte[]{1, 2, 3, 4, 5, 6}), 4, 8, 1000);
        assertArrayEquals(new byte[]{1, 2, 3, 4}, exact);

        Throwable shortRead = assertThrows(Throwable.class,
                () -> readExact(server, new ByteArrayInputStream(new byte[]{1, 2}), 4, 8, 1000));
        assertTrue(rootCause(shortRead) instanceof EOFException);

        Throwable capExceeded = assertThrows(Throwable.class,
                () -> readExact(server, new ByteArrayInputStream(new byte[]{1, 2, 3, 4}), 9, 8, 1000));
        assertTrue(rootCause(capExceeded) instanceof IOException);
        assertTrue(rootCause(capExceeded).getMessage().contains("exceeds cap"));
    }
    ...
    private static byte[] readExact(VotifierServer server, InputStream in, int expected, int cap, int timeoutMs) throws Exception {
        Method method = VotifierServer.class.getDeclaredMethod(
                "readExactWithDeadline", InputStream.class, int.class, int.class, int.class
        );
        method.setAccessible(true);
        try {
            return (byte[]) method.invoke(server, in, expected, cap, timeoutMs);
