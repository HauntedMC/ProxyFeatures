...
class ServerQueueTest {
    ...
    @Test
    void defensivePathsHandleSparseBucketsAndStaleIndexEntries() throws Exception {
        ServerQueue queue = new ServerQueue("queue");
        NavigableMap<Integer, Deque<QueueEntry>> buckets = getPrivateField(queue, "buckets");
        Map<UUID, QueueEntry> index = getPrivateField(queue, "index");

        buckets.put(5, new ArrayDeque<>());
        assertNull(queue.pollNextConnectable());

        UUID ghost = UUID.randomUUID();
        index.put(ghost, new QueueEntry(ghost, 9, Instant.now()));
        assertTrue(queue.positionOf(ghost).isEmpty());
    }
